...
import com.badlogic.gdx.backends.webgpu.lwjgl3.WebGPUGraphics.WebGPUMonitor;
...
public class WebGPUApplicationConfiguration extends WebGPUWindowConfiguration {
	...
	public static DisplayMode getDisplayMode (Monitor monitor) {
		WebGPUApplication.initializeGlfw();
		GLFWVidMode videoMode = GLFW.glfwGetVideoMode(((WebGPUMonitor)monitor).monitorHandle);
		return new WebGPUGraphics.WebGPUDisplayMode(((WebGPUMonitor)monitor).monitorHandle, videoMode.width(), videoMode.height(),
			videoMode.refreshRate(), videoMode.redBits() + videoMode.greenBits() + videoMode.blueBits());
	}
	...
	public static DisplayMode[] getDisplayModes (Monitor monitor) {
		WebGPUApplication.initializeGlfw();
		Buffer videoModes = GLFW.glfwGetVideoModes(((WebGPUMonitor)monitor).monitorHandle);
		DisplayMode[] result = new DisplayMode[videoModes.limit()];
		for (int i = 0; i < result.length; i++) {
			GLFWVidMode videoMode = videoModes.get(i);
			result[i] = new WebGPUGraphics.WebGPUDisplayMode(((WebGPUMonitor)monitor).monitorHandle, videoMode.width(),
	...
	static WebGPUMonitor toWebGPUMonitor (long glfwMonitor) {
		...
		return new WebGPUMonitor(glfwMonitor, virtualX, virtualY, name);
	}

	static GridPoint2 calculateCenteredWindowPosition (WebGPUMonitor monitor, int newWidth, int newHeight) {
		IntBuffer tmp = BufferUtils.createIntBuffer(1);
		IntBuffer tmp2 = BufferUtils.createIntBuffer(1);
