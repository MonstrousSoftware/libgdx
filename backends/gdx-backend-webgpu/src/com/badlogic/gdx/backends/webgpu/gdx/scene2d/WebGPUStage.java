...
public class WebGPUStage extends Stage {
	...
	public WebGPUStage(Viewport viewport) {
		this(viewport, new WebGPUSpriteBatch());
		ownsBatch = true;
	}
	...
	public WebGPUStage(Viewport viewport, Batch batch) {
	...
	private void drawDebug () {
		...
		if (debugUnderMouse || debugParentUnderMouse || debugTableUnderMouse != Debug.none) {
			...
			if (debugParentUnderMouse && actor.getParent() != null) actor = actor.getParent();
	...
	/** Applies a key down event to the actor that has {@link WebGPUStage#setKeyboardFocus(Actor) keyboard focus}, if any, and returns
	...
	/** Applies a key up event to the actor that has {@link WebGPUStage#setKeyboardFocus(Actor) keyboard focus}, if any, and returns true
	...
	/** Applies a key typed event to the actor that has {@link WebGPUStage#setKeyboardFocus(Actor) keyboard focus}, if any, and returns
	 * true if the event was {@link Event#handle() handled}. */
	...
	public void setRoot (Group root) {
		if (root.getParent() != null) root.getParent().removeActor(root, false);
		this.root = root;
		root.setParent(null);
		root.setStage(this);
	}
