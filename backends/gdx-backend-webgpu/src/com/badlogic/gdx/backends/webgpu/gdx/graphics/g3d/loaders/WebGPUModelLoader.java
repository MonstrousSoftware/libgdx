...
package com.badlogic.gdx.backends.webgpu.gdx.graphics.g3d.loaders;
...
import com.badlogic.gdx.backends.webgpu.gdx.graphics.g3d.WebGPUModel;
import com.badlogic.gdx.backends.webgpu.gdx.graphics.utils.WebGPUTextureProvider;
...
public abstract class WebGPUModelLoader<P extends WebGPUModelLoader.ModelParameters> extends AsynchronousAssetLoader<Model, P> {
	public WebGPUModelLoader(FileHandleResolver resolver) {
		super(resolver);
	}
	...
	public Model loadModel (final FileHandle fileHandle, TextureProvider textureProvider, P parameters) {
		...
		return data == null ? null : new WebGPUModel(data, textureProvider);
	}

	/** Directly load the model on the calling thread. The model with not be managed by an {@link AssetManager}. */
	public Model loadModel (final FileHandle fileHandle, P parameters) {
		return loadModel(fileHandle, new WebGPUTextureProvider.FileTextureProvider(), parameters);
	}
	...
	public Model loadModel (final FileHandle fileHandle) {
		return loadModel(fileHandle, new WebGPUTextureProvider.FileTextureProvider(), null);
	}
